...
class dcmrcv(object):
    def __init__(self, AETitle, port, RCVDIR, dcmrcvpath='dcmrcv', rspTO=6000000, requestTO=6000000, acceptTO=6000000, idleTO=6000000, native=True):
        self.AETitle = AETitle
        self.port = port
        self.RCVDIR = RCVDIR #directory where DICOM images are placed
        self.args = [dcmrcvpath, self.AETitle + ':' + self.port, '-dest', self.RCVDIR, '-rspTO', str(rspTO), '-requestTO', str(requestTO), '-acceptTO', str(acceptTO), '-idleTO', str(idleTO)]
        if native:
            self.args.append('-native')
        self.__checkdirs__()
